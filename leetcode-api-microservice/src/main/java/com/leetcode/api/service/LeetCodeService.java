    ...
    private LeetCodeStats parseResponse(Map body, String username) {
        ...
        try {
            ...
            if (recentSubmissions != null) {
                List<String> uniqueSlugs = new ArrayList<>();
            ...
            Map<String, Set<String>> seenEasy = new HashMap<>();
            Map<String, Set<String>> seenMedium = new HashMap<>();
            Map<String, Set<String>> seenHard = new HashMap<>();
            ...
            for (Map.Entry<String, String> entry : slugToDifficulty.entrySet()) {
                ...
                if ("Easy".equals(diff)) seenEasy.computeIfAbsent(lang, k -> new HashSet<>()).add(slug);
                else if ("Medium".equals(diff)) seenMedium.computeIfAbsent(lang, k -> new HashSet<>()).add(slug);
                else if ("Hard".equals(diff)) seenHard.computeIfAbsent(lang, k -> new HashSet<>()).add(slug);
            }
            ...
            if (languageProblemCount != null) {
                ...
                List<LanguageProgress> langStats = new ArrayList<>();
                Set<String> knownLanguages = new HashSet<>(List.of("Java", "C++", "Python", "Python3", "JavaScript", "MySQL"));
                ...
                for (String name : knownLanguages) {
                    ...
                    int easyS = Math.max(
                        seenEasy.getOrDefault(name, Collections.emptySet()).size(),
                        seenEasy.getOrDefault(langKey, Collections.emptySet()).size()
                    );
                    int mediumS = Math.max(
                        seenMedium.getOrDefault(name, Collections.emptySet()).size(),
                        seenMedium.getOrDefault(langKey, Collections.emptySet()).size()
                    );
                    int hardS = Math.max(
                        seenHard.getOrDefault(name, Collections.emptySet()).size(),
                        seenHard.getOrDefault(langKey, Collections.emptySet()).size()
                    );

                    LanguageProgress stat = new LanguageProgress(
                        name, solved, totalT,
                        easyS, mediumS, hardS,
                        easyT, mediumT, hardT
                    );
